...
def remove(serv, chan, nick):
	c = sqlite3.connect(settings.datafile_later)
	c.execute("DELETE FROM later WHERE server = '{0}' AND channel = '{1}' AND nick_to = '{2}'".format(serv, chan, nick))
	c.commit()
	c.close()
...
def count(serv, chan, nick_from, nick_to, msg):
	...
	c = sqlite3.connect(settings.datafile_later)
	for row in c.execute("SELECT * FROM later WHERE server = '{0}' AND channel = '{1}' AND nick_to = '{2}' AND msg = '{3}'".format(serv, chan, nick_to, msg)):
